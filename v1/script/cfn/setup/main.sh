...
LAMBDA_ACTION_MODE="CREATE_UPDATE"
...
echo ""
echo "If you would like to set all of the CloudFormation actions (for the lambda) to a different action mode, then set that region next."
read -p "CloudFormation Service Action Mode (Default: $LAMBDA_ACTION_MODE): " lambda_action_mode
echo "The CloudFormation Service Action Mode is: ${lambda_action_mode:-$LAMBDA_ACTION_MODE}"
...
if [ -n "${nonprod_profile:-$NONPROD_PROFILE}" ]; then
  ...
  run_deploy "${stack_name:-$STACK_NAME}$STACK_NAME_SUFFIX" "AppGitHubOrganization=${github_organization:-$GITHUB_ORGANIZATION} AppSourceRepository=${app_source_repository:-$APP_SOURCE_REPOSITORY} ExternalArtifactBucket=${prod_s3_artifact_bucket:-$PROD_S3_ARTIFACT_BUCKET} EnableCustomBuild=${nonprod_enable_custom_commands:-$NONPROD_ENABLE_CUSTOM_COMMANDS} ProjectName=${project_name:-$PROJECT_NAME} ServiceSubdomain=${service_subdomain:-$SERVICE_SUBDOMAIN} ActionMode=${action_mode:-$ACTION_MODE} LambdaActionMode=${lambda_action_mode:-$LAMBDA_ACTION_MODE} SecondRegion=${second_region:-$SECOND_REGION} ProdEnvironment=NONE TagEnvironment=nonprod " "$TAGS environment=nonprod" "${nonprod_profile:-$NONPROD_PROFILE}"
  if [ "${first_run:-$FIRST_RUN}" = "Yes" ]; then
